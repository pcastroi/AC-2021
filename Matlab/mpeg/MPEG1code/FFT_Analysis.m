...
%
%
%    Modified 05-04-02  Finn Agerkvist  :   normalization changed
%-------------------------------------------------------------------------------
Common;
...
X = max(20 * log10(abs(fft(s .* h)) / (FFT_SIZE*20e-6)), MIN_POWER);

% Normalization to the reference sound pressure level of 96 dB
%Delta = 96 - max(X);
%X = X +  Delta;

% Normalization corresponding to +/- 1 equal to 1 Pascal (94dB)  /FA